...
NUM_EPOCHS  = 20  # used only for x-axis labels if you pass training curves
...
print(f"\n{'='*60}")
print(f"  ÉVALUATION COMPLÈTE — TEST SET")
print(f"{'='*60}")
...
# ══════════════════════════════════════════════════════════════
# FIGURE 1 — Matrice de confusion (brute + normalisée)
# ══════════════════════════════════════════════════════════════
...
axes[0].set_title('Matrice de Confusion (brute)', fontsize=13, fontweight='bold')
axes[0].set_xlabel('Prediction'); axes[0].set_ylabel('Verite')

sns.heatmap(cm_norm, annot=True, fmt='.2%', cmap='Greens',
            xticklabels=CLASS_NAMES, yticklabels=CLASS_NAMES, ax=axes[1])
axes[1].set_title('Matrice de Confusion (normalisee)', fontsize=13, fontweight='bold')
axes[1].set_xlabel('Prediction'); axes[1].set_ylabel('Verite')

plt.tight_layout()
plt.savefig('fig1_confusion_matrix.png', dpi=120, bbox_inches='tight')
plt.show()
print("\n✓ fig1_confusion_matrix.png")

# ══════════════════════════════════════════════════════════════
# FIGURE 2 — Métriques par classe (bar chart)
# ══════════════════════════════════════════════════════════════
...
ax.set_title('Precision / Recall / F1-Score par classe', fontsize=13, fontweight='bold')
ax.legend(); ax.grid(axis='y', alpha=0.3)
plt.tight_layout()
plt.savefig('fig2_metrics_per_class.png', dpi=120, bbox_inches='tight')
plt.show()
print("✓ fig2_metrics_per_class.png")

# ══════════════════════════════════════════════════════════════
# FIGURE 3 — Courbes ROC (une par classe)
# ══════════════════════════════════════════════════════════════
...
for i, (cls, color) in enumerate(zip(CLASS_NAMES, COLORS)):
    ...
    axes[i].set_title(f'ROC - {cls.upper()}', fontsize=13, fontweight='bold', color=color)
...
plt.suptitle('Courbes ROC par Classe (One-vs-Rest)', fontsize=16, fontweight='bold')
plt.tight_layout()
plt.savefig('fig3_roc_per_class.png', dpi=130, bbox_inches='tight')
plt.show()
print("✓ fig3_roc_per_class.png")

# ══════════════════════════════════════════════════════════════
# FIGURE 4 — Courbes Precision-Recall (une par classe)
# ══════════════════════════════════════════════════════════════
fig, axes = plt.subplots(2, 2, figsize=(14, 11))
axes = axes.flatten()
...
for i, (cls, color) in enumerate(zip(CLASS_NAMES, COLORS)):
    ...
    axes[i].set_title(f'Precision-Recall - {cls.upper()}', fontsize=13,
...
plt.suptitle('Courbes Precision-Recall par Classe', fontsize=16, fontweight='bold')
plt.tight_layout()
plt.savefig('fig4_pr_per_class.png', dpi=130, bbox_inches='tight')
plt.show()
print("✓ fig4_pr_per_class.png")

# ══════════════════════════════════════════════════════════════
# FIGURE 5 — Distribution des probabilités par classe
# ══════════════════════════════════════════════════════════════
...
for i, (cls, color) in enumerate(zip(CLASS_NAMES, COLORS)):
    ...
    axes[i].set_title(f'Distribution scores - {cls.upper()}', fontsize=13,
                      fontweight='bold', color=color)
...
plt.suptitle('Distribution des Probabilites par Classe', fontsize=16, fontweight='bold')
plt.tight_layout()
plt.savefig('fig5_prob_distributions.png', dpi=130, bbox_inches='tight')
plt.show()
print("✓ fig5_prob_distributions.png")

# ══════════════════════════════════════════════════════════════
# FIGURE 6 — Dashboard récapitulatif global
# ══════════════════════════════════════════════════════════════
...
plt.suptitle('Dashboard - Evaluation Complete BrainTumorCNN',
             fontsize=17, fontweight='bold', y=1.01)
plt.savefig('fig6_dashboard.png', dpi=140, bbox_inches='tight')
plt.show()
...
print(f"{'='*55}")
for fname, desc in [
    ("fig1_confusion_matrix.png",    "Matrices de confusion brute + normalisee"),
    ("fig2_metrics_per_class.png",   "Precision / Recall / F1 par classe"),
    ("fig3_roc_per_class.png",       "Courbes ROC (1 graphe / classe)"),
    ("fig4_pr_per_class.png",        "Courbes Precision-Recall / classe"),
    ("fig5_prob_distributions.png",  "Distributions des probabilites"),
    ("fig6_dashboard.png",           "Dashboard recapitulatif global"),
]:
